...
function_dataDict = get_function_sign()
print(type(function_dataDict))

for key,value in function_dataDict.items():
    print(value)
    if "static" not in value.keys() :
        if(value['ret_type'] != "void"):
            
            if resolve(value['ret_type'].replace("<0x","").replace(">","")) is "Error":
                logError.append(value['name']+'....'+value['ret_type'])

        else:
            print("fixed return..",value)
        for parameters in value['param']:
            if resolve(parameters[1].replace("<0x","").replace(">","") ) is "Error":
